...
export default {
  ...
  setup() {
    ...
    const selectedSubjects = computed({
      get() {
        return subjects.value.split(",").filter((s) => s);
      },
      set(newValue) {
        subjects.value = newValue.join(",");
      },
    });

    const selectedAreas = computed({
      get() {
        return areas.value.split(",").filter((a) => a);
      },
      set(newValue) {
        areas.value = newValue.join(",");
      },
    });
    ...
    const getJobInformation = () => {
      error_message.value = "";
      $.ajax({
        url: "http://127.0.0.1:3000/teacher/get/job/information/",
        type: "get",
        headers: {
        ...
        success(resp) {
          if (resp.success == true) {
            ...
            certificate.value = resp.data.certificate;
            description.value = resp.data.description;
            areas.value = resp.data.areas;
            studentEvaluate.value = resp.data.studentEvaluate;
            mode.value = resp.data.mode;
            salary.value = resp.data.salary;
    ...
    const updateJobInformation = () => {
      ...
      error_message.value = "";
      $.ajax({
        url: "http://127.0.0.1:3000/teacher/update/job/information/",
        type: "post",
        headers: {
          Authorization: "Bearer " + store.state.teacher.token,
        },
        ...
        success(resp) {
          if (resp.success == true) {
            modal_error_message.value = "更新求职信息成功!";
